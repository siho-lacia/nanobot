...
class AgentLoop:
    ...
    def _parse_text_tool_calls(self, content: str | None) -> list[ToolCallRequest]:
        ...
        # Pattern matches: {"name": "toolname", "arguments": {...}}
    ...
    def _remove_text_tool_calls(self, content: str | None) -> str:
    ...
    async def _run_agent_loop(
        ...
        while iteration < self.max_iterations:
            ...
            if response.has_tool_calls:
                ...
                text_tool_calls = self._parse_text_tool_calls(clean)

                if text_tool_calls:
                    # Found text-based tool calls - treat them like structured calls
                    logger.info(
                        "Parsed {} text-based tool calls from response",
                        len(text_tool_calls),
                    )
                    if on_progress:
                        if clean and not any(
                            tc.name in ("read_file", "write_file", "edit_file", "list_directory")
                            for tc in text_tool_calls
                        ):
                            filtered = self._remove_text_tool_calls(clean)
                            if filtered.strip():
                                await on_progress(filtered)
                        await on_progress(self._tool_hint(text_tool_calls), tool_hint=True)
                    ...
                    filtered_content = self._remove_text_tool_calls(clean)
                    messages = self.context.add_assistant_message(
                        messages,
                        filtered_content if filtered_content.strip() else None,
                        tool_call_dicts,
                        reasoning_content=response.reasoning_content,
                        thinking_blocks=response.thinking_blocks,
                    )
