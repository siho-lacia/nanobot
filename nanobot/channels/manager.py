...
class ChannelManager:
    ...
    def _init_channels(self) -> None:
        ...
        # Zulip channel
        if self.config.channels.zulip.enabled:
            try:
                from nanobot.channels.zulip import ZulipChannel

                self.channels["zulip"] = ZulipChannel(
                    self.config.channels.zulip,
                    self.bus,
                )
                logger.info("Zulip channel enabled")
            except ImportError as e:
                logger.warning("Zulip channel not available: {}", e)
